...
namespace Engine
{
	...
	void SetSpriteSize(sf::Sprite& sprite, float desiredWidth, float desiredHeight)
	{
		...
		assert(spriteRect.width > 0.0f && "Sprite width is zero - cannot set size!");
		assert(spriteRect.height > 0.0f && "Sprite height is zero - cannot set size!");

		if (spriteRect.width <= 0.0f || spriteRect.height <= 0.0f)
		{
			LOG_ERROR("Sprite has invalid dimensions: " + std::to_string(spriteRect.width) +
				"x" + std::to_string(spriteRect.height));
			throw std::runtime_error("Sprite::SetSpriteSize - invalid sprite dimensions");
		}
	...
	void SetSpriteRelativeOrigin(sf::Sprite& sprite, float originX, float originY)
	{
		...
		assert(spriteRect.width > 0.0f && "Sprite width is zero - cannot set origin!");
		assert(spriteRect.height > 0.0f && "Sprite height is zero - cannot set origin!");

		if (spriteRect.width <= 0.0f || spriteRect.height <= 0.0f)
		{
			LOG_ERROR("Sprite has invalid dimensions for origin: " + std::to_string(spriteRect.width) +
				"x" + std::to_string(spriteRect.height));
			return;
		}
