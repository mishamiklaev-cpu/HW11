...
namespace Engine
{
    ...
    Matrix2D Matrix2D::GetInversed() const
    {
        ...
        assert(std::abs(determinant) > 1e-6f && "Matrix determinant is zero - cannot invert!");

        if (std::abs(determinant) < 1e-6f)
        {
            LOG_ERROR("Matrix determinant is zero - cannot invert!");
            throw std::runtime_error("Matrix2D::GetInversed - determinant is zero");
        }
    ...
    void Matrix2D::Print() const
    {
        ...
        for (int row = 0; row < 3; ++row) {
            logMsg += "| ";
            for (int col = 0; col < 3; ++col) logMsg += std::to_string(m[row][col]) + " ";
            logMsg += "|\n";
        }
        LOG_INFO(logMsg);
    }
